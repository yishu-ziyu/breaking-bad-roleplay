...
## 2026-09-09 Supabase Auth 恢复与部署配置修复

- 控制台确认原 abq-roleplay 项目为 paused，执行 Resume 后恢复 Healthy；未创建新项目。
- 新发现：VM 构建没有传 Supabase build args，运行容器也没有 Auth 配置。新增 `scripts/deploy-vm.sh`，读取 VM 已有公开配置，分别注入前端构建和后端 token 校验。生产完成重建并通过健康检查。
- 上传包排除 `.env*`，防止本地配置覆盖生产；构建失败不再继续删除运行容器，健康检查失败返回非零。
- 剧情数据库仍为 VM 的 `bb-postgres`，未合并或切回旧库。本地公开 Auth 配置同步完成，8001 后端重新启动，5176 代理健康检查通过。
- 恢复旧库时发现 5 张后台表无 RLS；通过 SQL Editor 应用 `20260909033000_lock_backend_tables.sql`。后台表禁止匿名和玩家 API 访问，三张云档案表保留原有用户隔离策略。
- 验证：前端 114 tests、后端 571 tests、Docker 构建、shell 语法检查通过；新增 2 项部署回归测试先红后绿。浏览器实走中文开场、选角、真实剧情生成、402 可读提示、重载恢复；临时账号密码登录、生产后端用户额度识别、三张档案表读写及跨用户隔离均通过，临时账号和数据已清理。个人玩家账号的浏览器登录未代为操作。
