...
- 对外展示以「素材库架构 + 安全边界」为主，不以 AMC/Sony 剧照、剧本或其他受版权保护内容作为主视觉。
...
5. Recap Craft Layer：第三方解说研究层（本地，非 canon）
   - 路径：`recap-murphy/`。
   - 内容：墨菲一口气解说的 ASR 与自写结构笔记。
   - 用于口播节奏、冷开、集循环、彩蛋附录；**禁止**当角色台词或写入检索层。

6. Character Intelligence Layer：时代切片决策层（运行时）
